...
@section('content')
            ...
            <form action="" method="post">
                ...
                <div id="step_2" class="step">
                    <div  class="cadastrar-parametros">
                        ...
                        <select class="form-select"  name="type_parameters" aria-label="Tipo">
                    ...
                    <div   class="cadastrar-parametros-gerencia">
                        <div class="row mb-4">
                            ...
                            <div class="col">
                                <div class="form-outline">
                                    ...
                                    <select class="form-select" name="" id="unidade" aria-label="Unidade">
                                        <option selected>Selecione a unidade</option>
                                        @foreach($units as $unit)
                                            <option name="{{$unit->id}}" codigoUnidade="{{$unit->cod}}">{{$unit->name}}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                            <div class="col">
                                <div class="form-outline">
                                    ...
                                    <input type="text" value="" class="form-control" name="" id="codigo_unidade">
                        ...
                        <div class="mb-3 col-3">
                            ...
                            <div class="form-group multiple-form-group input-group">
                                ...
                                <!--Input de pesquisa-->
                                <input name="upg" id="upg" class="form-control">
                                <button type="button" class="btn btn-success btn-addTeste" onclick="upgs()">+</button>
                                <!--resultado -->
                                <div class="col-12" id="inputsList">
    ...
    <script>
        $(`#unidade`).change(function(){
            let option = $(`option:selected`, this).attr(`codigoUnidade`);
            $(`#codigo_unidade`).val(option);
            $(`#codigo_unidade`).prop('disabled', true);

        });
        async function upgs() {
            //Buscar dados do input upgName
            let upgName = $("#upg").val();
            let _token = "{{ csrf_token() }}";
            //fazer a consulta na ROTA de UPG pelo nome enviado
            const { status, upgResult } = await $.post('load_upgs', {upgName, _token});

            if (status) {
                //busca os dados no localStorage e converte para array
                let upgsStorage = JSON.parse(localStorage.getItem("upgsStorage") || "[]");

                //Adiciona o novo dado no localStorage
                upgsStorage.push(...upgResult);
                //Salva o localStorage com o novo dado
                localStorage.setItem("upgsStorage", JSON.stringify(upgsStorage));

                //Refaz a leitura do localStorage com os novos dados inputados
                let upgsStorageRead = JSON.parse(localStorage.getItem("upgsStorage") || "[]");


                let upgHtml = '';
                //Faz a varredura na array do localStora para montar os inputs com os dados
                upgsStorageRead.forEach(function(upg, index) {
                    upgHtml += `  <div class="col-12">
                            <table class="table">
                                <thead>
                                <tr>
                                ...
                                <tbody>
                                 <tr>
                                       ...
                                       <td>
                                           <div class="form-check form-switch">
                                               <input class="form-check-input" name="${upg.status}" type="checkbox" id="flexSwitchCheckChecked" checked>
                                               <label class="form-check-label" for="flexSwitchCheckChecked"></label>
                                           </div>
                                       </td>
                ...
                //Exibi o html montado na div id inputsList
                $(`#inputsList`).html(upgHtml);

                return true;
            }

            console.log('Ops nenhum resultado');
            return false;
        }
